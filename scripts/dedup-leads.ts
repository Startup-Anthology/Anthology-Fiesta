 ...
 *
 * Usage (from repo root): node --env-file=.env --import tsx/esm scripts/dedup-leads.ts
 *    or via api-server: pnpm --filter api-server dev:fast (set DATABASE_URL first)
 *
 * Pass --execute to actually make changes (dry-run by default).
 *
 * Run from repo root: npx tsx --env-file=.env scripts/dedup-leads.ts
 */
...
// Step 2: Execute (drizzle doesn't support raw transactions easily, so we use sql tagged template)
for (const row of rows) {
  const [winner, ...losers] = row.ids;
