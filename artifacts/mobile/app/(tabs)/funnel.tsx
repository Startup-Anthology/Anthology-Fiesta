...
export default function FunnelScreen() {
  ...
  const params = useLocalSearchParams<{ segment?: string; contactTab?: string; filter?: string }>();
  const [segment, setSegment] = useState<"leads" | "contacts">(
    params.segment === "contacts" ? "contacts" : "leads"
  );
  ...
  if (isError) {
    return (
      <View style={[styles.container, { paddingTop: topPad, backgroundColor: colors.background }]}>
        <ErrorState message="Could not load pipeline." onRetry={refetch} />
      </View>
    );
  }
  // Contacts list data
    ...
    <View style={[styles.container, { paddingTop: topPad, backgroundColor: colors.background }]}>
      ...
      {params.filter === "week" && (
        <View style={[styles.filterBanner, { backgroundColor: colors.info + "15" }]}>
          ...
          <Text style={[styles.filterBannerText, { color: colors.info }]}>New this week</Text>
              ...
              renderItem={({ item }) => (
                ...
                >
                  <View style={styles.listCardLeft}>
                    ...
                    <Text style={[styles.leadEmail, { color: colors.textSecondary }]}>{item.company || item.email || ""}</Text>
                  </View>
                  <View style={styles.listCardRight}>
                    <View style={[styles.statusBadge, { backgroundColor: (REL_COLORS[item.relationshipType] || colors.primary) + "20" }]}>
                      <Text style={[styles.statusText, { color: REL_COLORS[item.relationshipType] || colors.primary }]}>{item.relationshipType}</Text>
                    </View>
      ...
      <Pressable
        ...
        onPress={() => {
          Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
          if (isContactsSegment) setShowAddContact(true);
          else setShowAdd(true);
        }}
        accessibilityRole="button"
        accessibilityLabel={isContactsSegment ? "Add contact" : "Add lead"}
      >
...
const styles = StyleSheet.create({
  ...
  columnCount: { fontSize: 13, fontFamily: "HankenGrotesk_500Medium", paddingHorizontal: 8, paddingVertical: 2, borderRadius: Layout.badgeRadius },
  leadCard: { borderRadius: Layout.cardRadius, padding: Layout.cardPadding, marginBottom: Layout.cardGap, ...Layout.shadow.sm },
  leadCardHeader: { flexDirection: "row", alignItems: "center", gap: 6 },
  leadName: { fontSize: 15, fontFamily: "HankenGrotesk_600SemiBold", flex: 1 },
  leadEmail: { fontSize: 13, fontFamily: "HankenGrotesk_400Regular", marginTop: 2 },
  leadMeta: { flexDirection: "row", alignItems: "center", gap: 8, marginTop: 10 },
  statusBadge: { paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  statusText: { fontSize: 11, fontFamily: "HankenGrotesk_600SemiBold", textTransform: "uppercase" },
  betaBadge: { paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 },
  betaBadgeText: { fontSize: 10, fontFamily: "HankenGrotesk_700Bold" },
  sourceText: { fontSize: 12, fontFamily: "HankenGrotesk_400Regular" },
  ...
  cancelBtn: { fontSize: 16, fontFamily: "HankenGrotesk_400Regular" },
  saveBtn: { fontSize: 16, fontFamily: "HankenGrotesk_600SemiBold" },
  saveBtnDisabled: { opacity: 0.4 },
  formGroup: { marginBottom: 22 },
  formLabel: { fontSize: 13, fontFamily: "HankenGrotesk_600SemiBold", marginBottom: 8, textTransform: "uppercase" },
  input: {
    borderRadius: Layout.inputRadius,
  ...
  segBtnText: { fontSize: 13, fontFamily: "HankenGrotesk_500Medium" },
